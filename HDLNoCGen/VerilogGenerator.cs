...
namespace HDL_NoC_CodeGen
{
    static class VerilogGenerator
    {
        ...
        public static void Create_verilog_file(int n_count, int bit, string proj_directory)
        {
            string FileName = "select_data_" + n_count.ToString() + "_" + bit.ToString() + ".v";
            FileStream fs = new FileStream(proj_directory + @"\" + FileName, FileMode.Create, FileAccess.ReadWrite);
            StreamWriter sw = new StreamWriter(fs);

            sw.WriteLine("`define N " + bit.ToString());
            int n_count_bit = Convert.ToInt32(Math.Ceiling(Math.Log(n_count, 2)));
            sw.WriteLine("`define K " + n_count_bit.ToString());
            sw.WriteLine();
            sw.WriteLine("module select_data_" + n_count.ToString() + "_" + bit.ToString() + "(");
            sw.WriteLine("\tclk, sw_on, sw_sel_data, sw_sel_router, key_inc, key_dec,");

            sw.WriteLine();
            for (int i = 1; i <= n_count; i++)
            {
                sw.WriteLine("\tout_router" + i.ToString() + ",");
            }
            sw.WriteLine();

            sw.WriteLine("\thex_data, hex_router");
            sw.WriteLine(");");
            sw.WriteLine();

            sw.WriteLine("\tinput clk;");
            sw.WriteLine("\tinput sw_on;");
            sw.WriteLine("\tinput sw_sel_data;");
            sw.WriteLine("\tinput sw_sel_router;");
            sw.WriteLine("\tinput key_inc;");
            sw.WriteLine("\tinput key_dec;");
            sw.WriteLine();

            for (int i = 1; i <= n_count; i++)
            {
                sw.WriteLine("\toutput reg[`N-1:0] out_router" + i.ToString() + ";");
            }
            sw.WriteLine();
            sw.WriteLine("\toutput reg[6:0] hex_data;");
            sw.WriteLine("\toutput reg[6:0] hex_router;");
            sw.WriteLine();

            sw.WriteLine("\treg[`N-2:0] data = `N'b0;");
            sw.WriteLine("\treg[`N-2:0] router = `BIT'b00000;");
            sw.WriteLine("\treg flag_key_inc = 1'b0;");
            sw.WriteLine("\treg flag_key_dec = 1'b0;");
            sw.WriteLine();

            int all_data_size = bit * n_count;
            sw.Write("\treg[" + (all_data_size - 1).ToString() + ":0] out_to_router;");
            sw.WriteLine("");

            sw.WriteLine("\talways@(posedge clk)");
            sw.WriteLine("\tbegin");

            sw.WriteLine("\t\tif(sw_on == 1'b1)");
            sw.WriteLine("\t\tbegin");
            sw.WriteLine("\t\t\tout_to_router = (" + all_data_size.ToString() + "'b0 + {1'b1, data}) << router*`N;");
            sw.WriteLine();
            ...
            for (int i = 1; i <= n_count; i++)
            {
                int lo = bit * (i - 1);
                int hi = bit * i - 1;
                sw.WriteLine("\t\t\tout_router" + i.ToString() + "  = out_to_router[" + hi.ToString() + ":" + lo.ToString() + "];");
            }
            sw.WriteLine("\t\tend");

            sw.WriteLine("\t\telse");
            sw.WriteLine("\t\tbegin");
            sw.WriteLine("\t\t\tout_to_router = " + all_data_size.ToString() + "'b0;");
            sw.WriteLine("\t\tend");
            sw.WriteLine();

            FileStream fs2 = new FileStream("other_base_code.txt", FileMode.Open, FileAccess.Read);
